...
def main(argv=None) -> int:
    ...
    (out/"manifest.json").write_text(json.dumps(result.manifest.to_dict(),indent=2)+"\n")
    (out/"receipts.jsonl").write_text("".join(json.dumps(x,sort_keys=True)+"\n" for x in result.receipts))
    (out/"result.json").write_text(json.dumps({"decision":result.decision,"execution":result.execution,"inference_output":result.inference_output},indent=2)+"\n")
    print(f"RUN {result.manifest.status.upper()}: {result.manifest.run_id}")
    print(f"MANIFEST: {out/'manifest.json'}")
    print(f"RECEIPTS: {out/'receipts.jsonl'}")
    return 0 if result.manifest.status in {"completed","dry_run","denied","escalated"} else 1
